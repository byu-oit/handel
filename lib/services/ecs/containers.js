...
/**
 * Given a container config, get the logging configuration for that container
 * @param {Object} container  - container definition from Handel file
 * @param clusterName ECS cluster name
 * @returns {?Object} config object, or null
 */
function getLoggingForContainer(container, clusterName) {
    let logging = container.logging || {enabled: true};

    if (!logging.enabled) {
        return null;
    }
    let group = logging.group || clusterName;
    let prefix = logging.prefix || container.name;
...
exports.getContainersConfig = function (ownServiceContext, dependenciesDeployContexts, clusterName) {
    ...
    for (let container of serviceParams.containers) {
        ...
        containerConfig.logging = getLoggingForContainer(container, clusterName);
