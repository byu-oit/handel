...
const dynamodbCalls = require('../../aws/dynamodb-calls');
...
exports.deploy = function (ownServiceContext, ownPreDeployContext, dependenciesDeployContexts) {
    ...
    return getCompiledDynamoTemplate(stackName, ownServiceContext)
        ...
        .then(deployedStack => {
            if (ownServiceContext.params.tags) {
                let tableName = cloudFormationCalls.getOutput('TableName', deployedStack);
                let tableARN = buildTableARN(tableName);
                console.log(ownServiceContext.params.tags);
                return dynamodbCalls.tagTable(tableARN, ownServiceContext.params.tags)
                    .then(() => deployedStack)
            }
            else {
