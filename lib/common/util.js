...
exports.replaceTagInFile = function (listTag, filePath, fileName) {
  ...
  {
    fs.readFile(`${filePath}/${fileName}`,'utf8',(err,readData)=>
    {
      if(err) reject(err);
      else    resolv(readData);
    });
  })
  .then(parm=>
  {
    for(let tag of listTag) parm=parm.replace(tag.regex,tag.value);
    return new Promise((resolv,reject)=>
    {
      fs.writeFile(`${filePath}/${fileName}`,parm,(err,rc)=>
      {
        if(err) reject(err);
        else
        {
