...
/**
 * Returns the PreDeployContext for the service. If PreDeploy has not been run yet for
 * this service, this function should return an error. 
 * 
 * This is used by external references to get information about the PreDeploy phase of 
 * an external service
 * 
...
exports.getBindContextForExternalRef = function(ownServiceContext, ownPreDeployContext, externalRefServiceContext, externalRefPreDeployContext) {
    ...
    let sourceSg = externalRefPreDeployContext.securityGroups[0];
    if(!ownSg) {
    ...
    return ec2Calls.getSecurityGroup(ownSg.GroupName, accountConfig.vpc)
        .then(efsSecurityGroup => {
            if(ec2Calls.ingressRuleExists(efsSecurityGroup, EFS_PORT, EFS_PORT, EFS_SG_PROTOCOL, sourceSg)) {
                return new BindContext(ownServiceContext, externalRefServiceContext);
            }
            else {
                throw new Error(`EFS - Bind has not been run on external service ${externalRefServiceContext.serviceName}`);
            }
        });
}
...
/**
 * Returns the DeployContext for the service. If Deploy has not been run yet for
 * this service, this method should return an error.
 * 
 * This is used by external references to get information about the Deploy phase of 
 * an external service.
 * 
...
/**
 * Returns the ConsumeEventsContext for the service. If ConsumeEvents has not been run yet for
 * this service, this method should return an error.
 * 
 * This is used by external references to get information about the ConsumeEvents phase of 
 * an external service.
 * 
 * @param {ServiceContext} ownServiceContext - The ServiceContext of this service consuming events
 * @param {DeployContext} ownDeployContext - The DeployContext of this service consuming events
...
exports.getConsumeEventsContextForExternalRef = function(ownServiceContext, ownDeployContext, externalRefServiceContext, externalRefDeployContext) {
    return Promise.reject(new Error("The EFS serviec doesn't consume events from other services"));
}   
