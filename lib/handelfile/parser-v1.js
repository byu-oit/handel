        ...
        for(let serviceName in environmentDef) {
            ...
            if(serviceDef.dependencies) { //Analyze those services that declare dependencies
                for(let dependentServiceName of serviceDef.dependencies) {
                    if(dependentServiceName.startsWith("https://")) { //External reference, skip validation
                        continue;                        
                    }
        ...
        for(let serviceName in environmentDef) {
            ...
            if(serviceDef.event_consumers) {
                for(let eventConsumerService of serviceDef.event_consumers) {
                    ...
                    if(eventConsumerServiceName.startsWith("https://")) { //External reference, skip validation
                        continue;                        
                    }

                    //Make sure the event consumer service exists in the environment
                    if(!environmentDef[eventConsumerServiceName]) {
                        throw new Error(`Invalid Handel file: You declared an event consumer '${eventConsumerServiceName}' in the service '${serviceName}' that doesn't exist`);
                    }
                    ...
                    if(!supportedConsumerTypes.includes(eventConsumerServiceDef.type)) {
                        throw new Error(`Invalid Handel file: The '${eventConsumerServiceDef.type}' service type can't consume events from the '${serviceDef.type}' service type`);
                    }
                }
            }
        }
...
exports.validateHandelFile = function (handelFile, serviceDeployers) {
    ...
    checkEventConsumers(handelFile, serviceDeployers);
};
