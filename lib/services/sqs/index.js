...
exports.getPreDeployContextForExternalRef = function (externalRefServiceContext) {
    winston.info(`SQS - Getting PreDeployContext for external service`);
...
exports.getBindContextForExternalRef = function (externalRefServiceContext, externalRefPreDeployContext, dependentOfServiceContext, dependentOfPreDeployContext) {
    winston.info(`SQS - Getting BindContext for external service`);
...
exports.getConsumeEventsContextForExternalRef = function (ownServiceContext, ownDeployContext, externalRefServiceContext, externalRefDeployContext) {
    return new Promise((resolve, reject) => {
        winston.info(`SQS - Getting ConsumeEventsContext for service ${ownServiceContext.serviceName}`);
        let queueUrl = ownDeployContext.eventOutputs.queueUrl;
        let queueArn = ownDeployContext.eventOutputs.queueArn;
        let producerServiceType = externalRefServiceContext.serviceType;
        let producerArn;
        if (producerServiceType === 'sns') {
            producerArn = externalRefDeployContext.eventOutputs.topicArn;
        }
        else {
            return reject(new Error(`SQS - Unsupported event producer type given: ${producerServiceType}`));
        }
        ...
        sqsCalls.getSqsPermission(queueUrl, policyStatement)
            .then(permissionStatement => {
                if (permissionStatement) {
                    return resolve(new ConsumeEventsContext(ownServiceContext, externalRefServiceContext));
                }
                return reject(new Error(`SQS - ConsumeEvents not run for external service`));
            });
    });
}
