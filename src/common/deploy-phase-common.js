 ...
 */
exports.createCustomRole = function (trustedService, roleName, policyStatementsToConsume, accountConfig) {
...
exports.uploadFileToHandelBucket = function (diskFilePath, artifactPrefix, s3FileName, accountConfig) {
...
exports.uploadDirectoryToHandelBucket = function (directoryPath, artifactPrefix, s3FileName, accountConfig) {
    let zippedPath = `${os.tmpdir()}/${s3FileName}.zip`;
    return util.zipDirectoryToFile(directoryPath, zippedPath)
        .then(() => {
            return exports.uploadFileToHandelBucket(zippedPath, artifactPrefix, s3FileName, accountConfig)
                .then(s3ObjectInfo => {
...
exports.uploadDeployableArtifactToHandelBucket = function (serviceContext, pathToArtifact, s3FileName) {
    let accountConfig = serviceContext.accountConfig;
    ...
    if (fileStats.isDirectory()) { //Zip up artifact and upload it
        return exports.uploadDirectoryToHandelBucket(pathToArtifact, artifactPrefix, s3FileName, accountConfig);
    }
    else { //Is file (i.e. WAR file or some other already-compiled archive), just upload directly
        return exports.uploadFileToHandelBucket(pathToArtifact, artifactPrefix, s3FileName, accountConfig);
    }
}
