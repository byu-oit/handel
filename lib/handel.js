    ...
    if(!accountConfig || !environmentContext) {
        ...
        if(errors.length === 0) {
            ...
            return preDeployLifecycle.preDeployServices(serviceDeployers, environmentContext)
                ...
                .catch(err => {
                    return new EnvironmentDeployResult("failure", err.message);
                });
        }
        else {
...
exports.deploy = function(accountConfigFileName, handelFileFileName, environmentsToDeploy, deployVersion) {
    ...
    for(let environmentToDeploy of environmentsToDeploy) {
        envDeployPromises.push(deployEnvironment(accountConfig, serviceDeployers, handelFileFileName, environmentToDeploy, deployVersion));
    }
