...
function deployServicesInLevel(environmentContext, deployOrder, level) {
    ...
    for(let i = 0; i < currentLevelElements.length; i++) {
        ...
        let dependentServiceContexts = getDependentServiceContexts(serviceToDeploy, environmentContext);
        serviceDeployPromises.push(serviceToDeploy.deployer.deploy(serviceToDeploy, dependentServiceContexts));
    }
...
function deployServices(environmentContext, deployOrder) {
    ...
    for(let currentLevel = 0; deployOrder[currentLevel]; currentLevel++) {
        deployProcess = deployProcess
            .then(() => deployServicesInLevel(environmentContext, deployOrder, currentLevel)) //TODO - MAKE SURE WE ARE RETURNING THE CORRECT THING HERE
    }
