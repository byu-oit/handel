...
function getAutoScalingEbExtension(stackName, ownServiceContext) {
    ...
    let handlebarsParams = {
        stackName,
        scaleUpPolicy: null, //This one will replace the 
        scaleDownPolicy: null,
        otherScalingPolicies: []
    }
    ...
    for (let policyConfig of serviceParams.auto_scaling.scaling_policies) {
        ...
        //We do this funny thing of having a single "scaleUp" and "scaleDown" policy, then put all others in a list
        //because Beanstalk has default scale up and down policies that we want to overwrite first so they won't conflict
        //with whatever the user has defined in their scaling policies
        if(scalingPolicy.scaleUp && !handlebarsParams.scaleUpPolicy) {
            handlebarsParams.scaleUpPolicy = scalingPolicy;
        }
        else if(scalingPolicy.scaleDown && !handlebarsParams.scaleDownPolicy) {
            handlebarsParams.scaleDownPolicy = scalingPolicy;
        }
        else {
