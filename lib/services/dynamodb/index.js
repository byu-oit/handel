...
function hasAutoscaling(provisionedThroughput) {
...
function getThroughputConfig(throughput, defaultConfig) {
    ...
    if (throughput.read_capacity_units) {
        Object.assign(read, parseThroughputCapacity(throughput.read_capacity_units));
        read.target = throughput.read_target_utilization || defaultRead.throughput || DEFAULT_AUTOSCALING_TARGET_UTILIZATION;
    } else {
        Object.assign(
            read,
    ...
    if (throughput.write_capacity_units) {
        Object.assign(write, parseThroughputCapacity(throughput.write_capacity_units));
        write.target = throughput.write_target_utilization || defaultWrite.throughput || DEFAULT_AUTOSCALING_TARGET_UTILIZATION;
    } else {
        Object.assign(
            write,
            {fixed: DEFAULT_CAPACITY_UNITS, target: DEFAULT_AUTOSCALING_TARGET_UTILIZATION, scaled: false},
...
exports.deploy = function (ownServiceContext, ownPreDeployContext, dependenciesDeployContexts) {
    ...
    return getCompiledDynamoTemplate(stackName, ownServiceContext)
        ...
        .then(deployedStack => {
            if (!hasAutoscaling(ownServiceContext.params.provisioned_throughput)) {
