...
function constructRoleName(ownServiceContext) {
    return `${ownServiceContext.appName}-${ownServiceContext.environmentName}-${ownServiceContext.serviceName}-${ownServiceContext.serviceType}`
}
...
exports.consumeEvents = function (ownServiceContext, ownDeployContext, producerServiceContext, producerDeployContext) {
    return new Promise((resolve, reject) => {
        ...
        if(producerServiceType === 'dynamodb') {
            ...
            policyStatementsToConsume[0].Resource.push(`arn:aws:dynamodb:${accountConfig.region}:${accountConfig.account_id}:table/${tableName}/stream/*`)
            return iamCalls.attachStreamPolicy(constructRoleName(ownServiceContext), policyStatementsToConsume)
            .then(() => {
            ...
            .then(() => {
                ...
                if(lambdaConsumer) {
                    ...
                    return reject(Error('Consumer serviceName not found in ownServiceContext'))
                }
            })
        } else {
            let sourceArn;
...
exports.unDeploy = function (ownServiceContext) {
    return iamCalls.detachPoliciesFromRole(constructRoleName(ownServiceContext))
        .then(() => {
